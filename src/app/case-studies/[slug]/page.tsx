...
export default function CaseStudyArticlePage() {
  ...
  const article = caseStudiesData.find((a) => a.id === slug);
  const relatedArticles = caseStudiesData.filter((a) => a.id !== slug).slice(0, 2);
  ...
  if (!article) {
    return (
      <div className="min-h-dvh flex flex-col items-center justify-center bg-background text-foreground p-6">
        ...
        <p className="text-muted-foreground mb-8">The requested enterprise case study does not exist or has been archived.</p>
        <Link href="/case-studies" className="inline-flex items-center gap-2 text-primary font-bold hover:underline">
  ...
  const handleRelatedClick = (e: React.MouseEvent, relatedId: string) => {
    e.preventDefault();
    if (document.startViewTransition) {
      document.startViewTransition(() => {
        router.push(`/case-studies/${relatedId}`);
      });
    } else {
      router.push(`/case-studies/${relatedId}`);
    }
  ...
  const renderContent = (content: string) => {
    return content.split("\n\n").map((block, index) => {
      ...
      if (block.startsWith("### ")) {
        return (
          <h3 key={index} className="text-xl font-bold text-foreground tracking-tight mt-8 mb-4 text-left">
            {block.replace("### ", "")}
          </h3>
        );
      }
      ...
      if (block.startsWith("- ") || block.startsWith("* ")) {
        ...
        return (
          <ul key={index} className="list-disc list-inside space-y-3.5 my-6 pl-4 text-muted-foreground font-medium text-base text-left">
            {items.map((item, i) => (
      ...
      // Regular Paragraphs
      return (
        <p key={index} className="text-muted-foreground font-medium leading-relaxed my-6 text-base md:text-lg text-left">
          {block}
        </p>
      );
    });
  ...
  return (
    <div className="min-h-dvh bg-background pb-24">
        ...
        <div className="w-full container px-6 sm:px-8 mx-auto relative z-10">
          <div className="max-w-4xl mx-auto space-y-6">
            ...
            <div className="flex items-center gap-3 flex-wrap pt-2">
              <span className="text-[10px] font-black text-white uppercase tracking-widest bg-cyan-600 px-3 py-1 rounded-full border border-cyan-500/30">
                CASE STUDY
              </span>
            ...
            <div className="flex flex-wrap items-center gap-x-6 gap-y-3 text-xs font-semibold text-white/70 pt-6 border-t border-white/10">
              <span className="flex items-center gap-2">
                <Briefcase className="w-4 h-4 text-cyan-400" /> {article.industry}
              </span>
            </div>
          </div>
        </div>
      ...
      {/* Metrics Banner */}
      <section className="bg-background border-y border-border relative z-20 py-8 shadow-sm">
        <div className="w-full container px-6 sm:px-8 mx-auto max-w-4xl">
          <div className="grid grid-cols-3 gap-6 text-center">
            {article.metrics.map((m, idx) => (
              <div key={idx} className="space-y-1">
                <div className="text-3xl sm:text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-cyan-600 to-blue-600">
                  {m.value}
                </div>
              ...
              <nav className="flex flex-col space-y-3">
                {headings.map((h) => (
                  <a
                    ...
                    className={`text-xs font-bold leading-relaxed transition-all duration-300 hover:text-cyan-600 text-left ${
                      activeHeading === h.id
                        ? "text-cyan-600 pl-2 border-l-2 border-cyan-600"
          ...
          <main className="lg:col-span-9 bg-card border border-border p-8 sm:p-12 rounded-3xl shadow-[0_4px_30px_rgba(0,0,0,0.01)]">
            ...
            <div className="mt-16 pt-8 border-t border-border flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                <div className="bg-cyan-500/10 p-3 rounded-2xl text-cyan-600 dark:bg-cyan-500/20">
                  <User className="w-5 h-5" />
                </div>
                <div>
                  <span className="text-sm font-extrabold text-foreground block">Jeshurun Enterprise Solutions</span>
                  <span className="text-[11px] text-muted-foreground font-bold block">Delivery Assurance Team</span>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 text-[10px] text-muted-foreground font-black uppercase tracking-widest bg-slate-100 dark:bg-slate-900 border border-border px-4 py-1.5 rounded-full">
                <ShieldAlert className="w-4 h-4 text-cyan-600" /> MEASURABLE CASE STUDY OUTCOME
              </div>
            </div>
          </main>
        </div>
        ...
        <section className="max-w-6xl mx-auto mt-20 pt-16 border-t border-border/80">
          ...
          <div className="grid gap-6 md:grid-cols-2">
            {relatedArticles.map((rel) => (
              <div
                key={rel.id}
                onClick={(e) => handleRelatedClick(e, rel.id)}
                className="group relative flex flex-col bg-card border border-border rounded-3xl overflow-hidden transition-all duration-300 hover:border-cyan-500 hover:shadow-[0_12px_24px_rgba(6,182,212,0.05)] cursor-pointer"
              >
                {/* Header Image */}
                ...
                <div className="p-6 flex-1 flex flex-col justify-between text-left">
                  ...
                  <div className="flex items-center justify-between mt-6 pt-4 border-t border-border/60">
                    ...
                    <span className="inline-flex items-center gap-1 text-xs font-bold text-cyan-600">
                      Read Case Study <ArrowRight size={12} className="transition-transform duration-300 group-hover:translate-x-1" />
                    </span>
                  </div>
                </div>
              </div>
