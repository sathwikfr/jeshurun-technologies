...
export function AnimatedCounter({ target, suffix = "", prefix = "", delay = 0 }: { target: number | string, suffix?: string, prefix?: string, delay?: number }) {
...
const defaultStatsData: HeroStat[] = [
  {
    id: 1,
...
export const flagshipStatsData: HeroStat[] = [
  {
    id: 1,
    icon: <Briefcase className="w-4 h-4" strokeWidth={1.5} />,
    target: 150,
...
export function HeroStatsPanel({ stats }: { stats?: HeroStat[] }) {
      ...
      >
        <div className="grid grid-cols-2 md:grid-cols-4 divide-x divide-y md:divide-y-0 divide-black/8 dark:divide-white/8">
          {displayStats.map((stat, i) => (
            <motion.div
              key={stat.id}
              className={`group relative flex flex-col items-center justify-center p-4 lg:p-5 xl:p-6 transition-all duration-300 hover:bg-black/[0.03] dark:hover:bg-white/[0.03] min-w-0 border-l-2 border-l-transparent ${stat.borderHover || 'hover:border-l-primary'} cursor-default`}
              whileHover={{ y: -2 }}
              transition={{ duration: 0.2 }}
            >
              {/* Subtle stat-specific top accent line on hover */}
              <div className={`absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r ${stat.accentColor || 'from-primary to-accent'} opacity-0 group-hover:opacity-100 transition-opacity duration-300`} />
              ...
              <div className="flex flex-col xl:flex-row items-center xl:items-start gap-2.5 xl:gap-4 text-center xl:text-left min-w-0 w-full">
                {/* Icon Container */}
                {stat.icon && (
                  <div className={`relative flex items-center justify-center w-10 h-10 rounded-xl ${stat.iconBg || 'bg-primary/10'} shrink-0`}>
                    {/* Breathing glow */}
                    <motion.div
                      className={`absolute inset-0 rounded-xl bg-gradient-to-br ${stat.accentColor || 'from-primary to-accent'} blur-md`}
                      animate={{
                        scale: [1, 1.4, 1],
                    ...
                    />
                    <div className={`relative z-10 ${stat.iconColor || 'text-primary'}`}>{stat.icon}</div>
                  </div>
                )}
                ...
                <div className="flex flex-col min-w-0">
                  <div className="text-xl md:text-2xl font-extrabold text-slate-900 dark:text-white leading-none tracking-tight mb-1">
                    <AnimatedCounter target={stat.target} suffix={stat.suffix} prefix={stat.prefix} delay={stat.delay || (i * 150)} />
                  </div>
