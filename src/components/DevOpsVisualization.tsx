...
import { Workflow, Terminal, GitBranch, CheckCircle2 } from "lucide-react";

const BAR_HEIGHTS = [65, 95, 80, 100, 90, 75];
    ...
    >
      ...
      >
        ...
        <div className="pointer-events-none absolute inset-0 hidden dark:block">
          <div className="absolute top-0 left-1/2 -translate-x-1/2 w-64 h-24 rounded-full bg-emerald-500/10 blur-2xl" />
        </div>
        ...
        <div className="relative flex items-center justify-between px-4 pt-3.5 pb-2.5 border-b border-slate-100 dark:border-zinc-800/80">
          <div className="flex items-center gap-2.5">
            <div className="w-7 h-7 rounded-lg bg-emerald-50 dark:bg-emerald-500/15 border border-emerald-100 dark:border-emerald-500/30 flex items-center justify-center">
              <Workflow className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" />
            </div>
            <div>
              <p className="text-[10px] font-extrabold text-emerald-600 dark:text-emerald-400 uppercase tracking-widest leading-none mb-0.5">CI/CD & IaC Engine</p>
              <p className="text-xs sm:text-sm font-bold text-slate-900 dark:text-white leading-none tracking-tight">Automated Release Telemetry</p>
            </div>
          </div>
          {/* Active Indicator */}
          <div className="flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200/80 dark:border-emerald-500/30">
            ...
            <span className="text-[10px] font-extrabold text-emerald-700 dark:text-emerald-400 uppercase tracking-wider">Passing</span>
          </div>
        </div>

        {/* DevOps Nodes Rows */}
        <div className="relative px-4 py-2.5 space-y-2">
          {DEVOPS_NODES.map((node, i) => {
            ...
            return (
              ...
              >
                ...
                <div className="text-right shrink-0">
                  ...
                  <p className="text-xs sm:text-sm font-extrabold text-emerald-600 dark:text-emerald-400">{node.metric}</p>
                </div>
              </motion.div>
            );
          })}
        ...
        <div className="relative px-4 pb-3 pt-1 grid grid-cols-3 gap-2 border-t border-slate-100 dark:border-zinc-800">
          ...
          >
            <p className="text-xs sm:text-sm font-black text-slate-900 dark:text-white">8min</p>
          ...
          >
            <p className="text-xs sm:text-sm font-black text-slate-900 dark:text-white">99.8%</p>
            <p className="text-[9px] text-slate-500 dark:text-zinc-400 font-semibold mt-0.5">Build Success</p>
          </motion.div>
          <motion.div
            initial={{ opacity: 0, y: 6 }}
            animate={isInView ? { opacity: 1, y: 0 } : {}}
            ...
            className="text-center pt-2"
          >
            <p className="text-xs sm:text-sm font-black text-slate-900 dark:text-white">Instant</p>
            <p className="text-[9px] text-slate-500 dark:text-zinc-400 font-semibold mt-0.5">Rollback Speed</p>
          </motion.div>
        </div>
      </motion.div>
      ...
      <div className="grid grid-cols-2 gap-2.5">
        ...
        >
          <div className="mb-1.5">
            <span className="text-[9px] font-extrabold text-slate-500 dark:text-zinc-300 uppercase tracking-widest truncate block">Build Success</span>
          </div>
          ...
          <p className="text-[9px] text-slate-400 dark:text-zinc-400 font-medium mt-0.5">Automated CI Gates</p>
          {/* Sparkline */}
          <div className="mt-2 h-4.5 flex items-end gap-0.5">
            {[75, 85, 90, 95, 98, 95, 99, 98, 100].map((h, i) => (
              <motion.div
                key={i}
        ...
        >
          <div className="mb-1.5">
            <span className="text-[9px] font-extrabold text-slate-500 dark:text-zinc-300 uppercase tracking-widest truncate block">Deploy Cycle</span>
          </div>
          <div className="flex items-baseline gap-1">
            <span className="text-base sm:text-lg font-black text-slate-900 dark:text-white">8</span>
            <span className="text-xs font-bold text-blue-600 dark:text-blue-400">min avg</span>
          </div>
          <p className="text-[9px] text-slate-400 dark:text-zinc-400 font-medium mt-0.5">Terraform & Helm</p>
          {/* Animated bars */}
          <div className="mt-2 flex items-end gap-1 h-4.5">
            {BAR_HEIGHTS.map((h, i) => (
              <motion.div
                key={i}
                className="flex-1 bg-blue-600 dark:bg-blue-400 rounded-t-[2px]"
                initial={{ height: "20%" }}
                animate={{ height: `${h}%` }}
                transition={{
                  duration: 0.8,
